...
struct Inner {
    ...
    name_to_registration_callbacks: HashMap<String, Vec<rsbinder::Strong<dyn hub::android::os::IServiceCallback::IServiceCallback>>>,
    name_to_client_callbacks: HashMap<String, Vec<rsbinder::Strong<dyn hub::android::os::IClientCallback::IClientCallback>>>,
}
...
impl IServiceManager for ServiceManager {
    ...
    fn registerForNotifications(&self, name: &str, arg_callback: &rsbinder::Strong<dyn hub::android::os::IServiceCallback::IServiceCallback>) -> rsbinder::status::Result<()> {
        if !Self::is_valid_service_name(name) {
            return Err(ExceptionCode::IllegalArgument.into());
        }
    ...
    fn unregisterForNotifications(&self, name: &str, callback: &rsbinder::Strong<dyn hub::android::os::IServiceCallback::IServiceCallback>) -> rsbinder::status::Result<()> {
        let mut inner = self.inner.lock().unwrap();
    ...
    fn registerClientCallback(&self, name: &str, arg_service: &rsbinder::SIBinder, arg_callback: &rsbinder::Strong<dyn hub::android::os::IClientCallback::IClientCallback>) -> rsbinder::status::Result<()> {
        let mut inner = self.inner.lock().unwrap();
    ...
    fn getServiceDebugInfo(&self) -> rsbinder::status::Result<Vec<hub::android::os::ServiceDebugInfo::ServiceDebugInfo>> {
        ...
        for (name, service) in inner.name_to_service.iter() {
            out.push(hub::android::os::ServiceDebugInfo::ServiceDebugInfo {
                name: name.clone(),
